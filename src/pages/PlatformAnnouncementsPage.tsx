...
function CreateModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: () => void }) {
  ...
  useEffect(() => {
    if (open) { setTitle(""); setBody(""); setExpireDate(""); setExpireTime(""); setErr(null); }
  }, [open]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      let expiresAt = null;
      if (expireDate) {
        expiresAt = `${expireDate}T${expireTime || "23:59"}:00`;
      }
      await apiPost("/api/v1/platform/announcements", { title, body, expiresAt });
      onCreated();
      onClose();
    } catch (e) {
        ...
        <form onSubmit={(e) => void submit(e)} className="p-6 space-y-4">
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="block text-xs font-semibold text-slate-600 mb-1" htmlFor="an-expires-date">Expiration Date (optional)</label>
              <input id="an-expires-date" type="date" value={expireDate} onChange={(e) => setExpireDate(e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
            </div>
            <div className="flex-1">
              <label className="block text-xs font-semibold text-slate-600 mb-1" htmlFor="an-expires-time">Time</label>
              <input id="an-expires-time" type="time" value={expireTime} onChange={(e) => setExpireTime(e.target.value)} disabled={!expireDate} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 disabled:opacity-50 disabled:bg-slate-50" />
            </div>
          </div>
