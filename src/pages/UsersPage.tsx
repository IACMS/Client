      ...
      <div className="mb-8 flex justify-between items-end flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-2 text-slate-500 font-label-caps text-xs mb-2 flex-wrap">
            ...
            <span className="text-primary font-bold">{t("portal.breadcrumb.users")}</span>
          </div>
          <h1 className="font-h1 text-primary">{t("users.title")}</h1>
          <p className="font-body-md text-slate-600 mt-1">{t("users.subtitle")}</p>
        </div>
        {canInvite && (
          <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-outline-variant text-label-caps text-slate-500">
                  ...
                  <th className="p-4 font-semibold">{t("users.table.status")}</th>
                  <th className="p-4 font-semibold">{t("users.table.lastLogin")}</th>
                  {canEdit && <th className="p-4 font-semibold text-right">{t("users.table.actions")}</th>}
                </tr>
              </thead>
                  ...
                  <tr key={u.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4">
                      <span className="text-sm font-medium text-slate-700 capitalize">
                        {u.role ? u.role.name.replace('_', ' ') : t("users.roleNone")}
                      </span>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-sm text-slate-500">
                      {u.lastLogin ? new Date(u.lastLogin).toLocaleDateString() : t("users.lastLoginNever")}
                    </td>
                    {canEdit && (
